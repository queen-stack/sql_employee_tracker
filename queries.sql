...
-- View employees by manager
 SELECT employee.id, employee.first_name, employee.last_name, role.title, role.salary, department.name AS department_name, concat(manager.first_name, " ", manager.last_name) AS manager_full_name 
  ...
  WHERE manager.first_name like "Andrew" and department.name like 'Management';
...
DELETE FROM employee WHERE first_name like 'Marilyn' AND last_name like 'Pop';
...
select name, sum(r.salary) from  employees_db.department d
